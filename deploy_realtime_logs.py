#!/usr/bin/env python3
"""Deploy both backend (with SSE hub) and frontend (with logs page) - optimized tar.gz."""
import paramiko
import sys
import os
import time
...
def upload_tar_gz(ssh, local_dir, remote_dir):
    """Pack local_dir to tar.gz, upload via SFTP, extract on remote."""
    buf = io.BytesIO()
    with tarfile.open(fileobj=buf, mode='w:gz') as tar:
        for root, dirs, files in os.walk(local_dir):
            ...
            for f in files:
                ...
                if any(pat in rel.split("/") for pat in EXCLUDE_PATTERNS):
                    continue
                tar.add(local_path, arcname=rel)
    buf.seek(0)

    sftp = ssh.open_sftp()
    remote_tar = remote_dir + "/upload.tar.gz"
    sftp.putfo(buf, remote_tar)
    size_mb = buf.tell() / (1024 * 1024)
    print("  Archive: {:.1f} MB".format(size_mb))
    sftp.close()
...
def main():
    ...
    upload_tar_gz(ssh, "e:/IOTsys/server", "{}/server".format(DEPLOY_BASE))

    # Build
    print("\n>>> Building iot-server image...")
    out, err = run_ssh(ssh, "cd {}/server && docker build -t iot-server:latest . 2>&1".format(DEPLOY_BASE), timeout=300)
    print(safe(out))
    ...
    upload_tar_gz(ssh, "e:/IOTsys/web/admin", admin_remote)
